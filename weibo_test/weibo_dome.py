...
def get_infor(url):
    ...
    ##微博数
    str_wb = browser.find_element_by_xpath('//*[@id="Pl_Core_T8CustomTriColumn__3"]/div/div/div/table/tbody/tr/td[3]/strong')
    ...
    for value in guid:
        ...
        break
    # 关注数
    str_gz = browser.find_element_by_xpath('//*[@id="Pl_Core_T8CustomTriColumn__3"]/div/div/div/table/tbody/tr/td[1]/strong')
    guid = re.findall(pattern, str_gz.text, re.M)
    num_gz = int(guid[0])
    # 粉丝数
    str_fs = browser.find_element_by_xpath('//*[@id="Pl_Core_T8CustomTriColumn__3"]/div/div/div/table/tbody/tr/td[2]/strong')
    guid = re.findall(pattern, str_fs.text, re.M)
    num_fs = int(guid[0])

    # print("start save info...")
    # writer = csv.writer(open('weibo_info.csv', 'w'))
    ...
    print("存入csv....")
    weibo = pd.DataFrame(
        {'昵称': num_name, '微博数': num_wb, '关注数': num_gz, '粉丝数': num_fs})
    table = weibo.set_index('num_name')
    table.head()
    weibo.to_csv('info.csv', mode='a', header=False)
    print("done save csv....")
...
def get_comcent():
    ...
    table.head()
    weibo.to_csv('weibo_data.csv' ,mode='a', header=False)
    print("done save csv....")
